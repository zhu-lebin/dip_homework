...
def train_one_epoch(generator,dataloader,criterion, optimizer_g, device, epoch, num_epochs,output_dir,history,log_file_path):
    """
    ...
    print(f'Epoch [{epoch + 1}/{num_epochs}], Generator Loss: {avg_train_loss_g:.4f}\n')
    # 保存日志
    with open(log_file_path, "a") as log_file:
        log_file.write(f"Epoch [{epoch + 1}/{num_epochs}], Generator Loss: {avg_train_loss_g:.4f}\n")
...
def create_output_dir(base_dir="output-l1-2"):
...
def main():
    ...
    for epoch in range(num_epochs):
        train_one_epoch(generator, train_loader, criterion,optimizer_g, device, epoch, num_epochs,output_dir,history,log_file_path) 
        if epoch % 1 == 0 :
