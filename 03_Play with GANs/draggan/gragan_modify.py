...
with gr.Blocks() as app:
    ...
    def get_face_slim_points(landmarks):
        ...
        """
        points = {}
        left_face = landmarks[0:8]  
        right_face = landmarks[9:17]
        ...
        for i, point in enumerate(left_face):
            x, y = point
            points[idx] = {
                'target': [int(x+(9-i)*2), int(y)],# 左脸向右移动
                'start': [int(x), int(y)]  
            }
            idx += 1
        ...
        for i, point in enumerate(right_face):
            x, y = point
            points[idx] = {
                'target': [int(x-(i+2)*2), int(y)], # 右脸向左移动
                'start': [int(x), int(y)]  
            }
            idx += 1
    ...
    def get_eye_close_points(landmarks):
        """
        Args:
        ...
        points = {}
        # 获取左右眼关键点
        left_eye_up = landmarks[37:39]
        left_eye_down = landmarks[41:39:-1]
        right_eye_up = landmarks[43:45]
        right_eye_down = landmarks[47:45:-1]
        
        idx = 0
        for p1, p2 in zip(left_eye_up, left_eye_down):
            ...
            idx += 1
            points[idx] = {
                'start': [int(p2[0]), int(p2[1])],
                'target': [mid_x, mid_y]
            }
            idx += 1
        for p1, p2 in zip(right_eye_up, right_eye_down):
            mid_x = int((p1[0] + p2[0]) / 2)
            mid_y = int((p1[1] + p2[1]) / 2)
            points[idx] = {
                'start': [int(p1[0]), int(p1[1])],
    ...
    def get_smile_points(landmarks):
        """
        Args:
            landmarks: (68, 2) 关键点坐标
            offset: 控制嘴角上扬的像素值
        ...
        up_mouth = landmarks[51]
        bottom_mouth = landmarks[57]
        offsety=20
        offsetx=10
        points[0] = {
            'start': [int(left_mouth[0]), int(left_mouth[1])],
            'target': [int(left_mouth[0]-offsetx), int(left_mouth[1] - offsety)]
        }
        
        points[1] = {
            'start': [int(right_mouth[0]), int(right_mouth[1])],
            'target': [int(right_mouth[0]+offsetx), int(right_mouth[1] - offsety)]
        }
        
        points[2] = {
            'start': [int(bottom_mouth[0]), int(bottom_mouth[1])],
            'target': [int(bottom_mouth[0]), int(bottom_mouth[1])]
        }
        
        points[3] = {
            'start': [int(up_mouth[0]), int(up_mouth[1])],
            'target': [int(up_mouth[0]), int(up_mouth[1])]
        }
        
        return points

    def on_click_eye_close(global_state):
        """一键闭眼功能"""
        image = global_state['images']['image_raw']
        landmarks = get_face_landmarks(image)
        ...
        points = get_eye_close_points(landmarks)
        global_state['points'] = points
