...
def laplacian_kernel():
    ...
    kernel = torch.tensor([[0., 1., 0.],
                           [1., -4., 1.],
                           [0., 1., 0.]]).unsqueeze(0).unsqueeze(0).float()  # Shape: [1, 1, 3, 3]
    return kernel.repeat(3, 1, 1, 1)  # Repeat for 3 channels, shape: [3, 1, 3, 3]

# 提取 mask 区域边界
def extract_masked_region(tensor, mask):
    """提取 tensor 中 mask 区域的局部图像"""
    # 找到 mask 区域的边界坐标
    nonzero_indices = torch.nonzero(mask, as_tuple=True)
    min_y, max_y = nonzero_indices[2].min(), nonzero_indices[2].max()
    min_x, max_x = nonzero_indices[3].min(), nonzero_indices[3].max()
    # 根据边界抠出局部图像
    cropped_tensor = tensor[:, :, min_y:max_y+1, min_x:max_x+1]
    cropped_mask = mask[:, :, min_y:max_y+1, min_x:max_x+1]
    return cropped_tensor, cropped_mask

# 计算前景和融合图像之间的拉普拉斯损失，仅针对 mask 区域
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    """
    计算前景和融合图像在 mask 区域内的拉普拉斯损失。

    Args:
        foreground_img (torch.Tensor): 前景图像张量。
        foreground_mask (torch.Tensor): 前景遮罩张量。
        blended_img (torch.Tensor): 融合图像张量。
        background_mask (torch.Tensor): 背景遮罩张量。

    Returns:
        torch.Tensor: 计算的拉普拉斯损失。
    """
    # 提取局部图像
    masked_foreground, local_mask_fg = extract_masked_region(foreground_img, foreground_mask)
    masked_blended, local_mask_bl = extract_masked_region(blended_img, foreground_mask)

    # 计算局部图像的拉普拉斯
    laplacian_fg = torch.nn.functional.conv2d(masked_foreground, weight=laplacian_kernel(), padding=1, groups=3)
    laplacian_blended = torch.nn.functional.conv2d(masked_blended, weight=laplacian_kernel(), padding=1, groups=3)

    # 确保尺寸一致
    if laplacian_fg.shape != laplacian_blended.shape:
        laplacian_blended = torch.nn.functional.interpolate(laplacian_blended, size=laplacian_fg.shape[2:], mode='bilinear', align_corners=False)

    # 仅计算局部图像的拉普拉斯损失
    laplacian_loss = ((laplacian_fg - laplacian_blended) ** 2 * local_mask_fg).sum() / (local_mask_fg.sum() + 1e-6)
...
def blending(foreground_image_original, background_image_original, dx, dy, polygon_state):
    ...
    if not polygon_state['closed'] or background_image_original is None or foreground_image_original is None:
        return np.array(background_image_original)  # Return original background if conditions are not met
    ...
    for step in range(iter_num):
        ...
        if loss.item() < best_loss:
            best_loss = loss.item()
            patience_counter = 0  # Reset counter if loss improved
        else:
            patience_counter += 1

        # Check if we reached the patience limit
        if patience_counter >= patience:
            print(f'Early stopping at step {step}, no improvement in the last {patience} steps.')
            break
