...
class FullyConvNetwork(nn.Module):
    ...
    def __init__(self):
        ...
        self.enc1 = self.conv_block(3, 16, normalize=False)  # Downsample 1
        self.enc2 = self.conv_block(16, 32)                 # Downsample 2
        self.enc3 = self.conv_block(32, 64, dropout=True)  # Downsample 3 (Add Dropout)
        self.enc4 = self.conv_block(64, 128, dropout=True)  # Downsample 4 (Add Dropout)
        self.enc5 = self.conv_block(128, 128, dropout=True)  # Downsample 5 (Bottleneck, Add Dropout)

        # Decoder (Deconvolutional Layers with Skip Connections)
        self.dec1 = self.deconv_block(128, 128, dropout=True)  # Upsample 1
        self.dec2 = self.deconv_block(256, 64, dropout=True) # Upsample 2
        self.dec3 = self.deconv_block(128, 32)                # Upsample 3
        self.dec4 = self.deconv_block(64, 16)                 # Upsample 4
