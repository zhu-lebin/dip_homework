...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    ...
    laplacian_kernel = torch.tensor([[[[0, 1, 0], [1, -4, 1], [0, 1, 0]]]], device=foreground_img.device, dtype=foreground_img.dtype)
    # Ensure the kernel has the same number of channels as the input
    laplacian_kernel = laplacian_kernel.repeat(foreground_img.size(1), 1, 1, 1)
    # Compute the Laplacian of the foreground and blended images
    laplacian_foreground = F.conv2d(foreground_img, laplacian_kernel, padding=1, groups=foreground_img.size(1))
    laplacian_blended = F.conv2d(blended_img, laplacian_kernel, padding=1, groups=blended_img.size(1))
    
    # Compute the difference within the masks
    diff = (laplacian_foreground - laplacian_blended) ** 2
    masked_diff = diff * foreground_mask * background_mask
    
    # Compute the loss
    loss = masked_diff.sum() / (foreground_mask * background_mask).sum()
    return loss
